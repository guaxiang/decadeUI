app.import(function(lib, game, ui, get, ai, _status, app) {
    lib.ui.create.pause = function () {/*覆写历史记录*/
    ...
    lib.ui.click.pause = function () {/*覆写历史记录*/
        ...
        let allOptionAnniu = ui.create.div('.jiluanniu', allOptionBg, function (event){changeall(event)});
        ui.create.div('.gou', allOptionAnniu);
        let name=ui.create.div('.name','全部',namebgbg,function(event){changeall(event)});
        for (let tar of player_list) {
            ...
            let name=ui.create.div('.name',prefixName+seatnum,namebgbg,function(event){change(event)});
            let anniu=ui.create.div('.jiluanniu',namebg,function(event) {change(event)})
        };
	...
	lib.arenaReady.push(function() {
		...
		game.ui_identityShow_update=function(){//左上角整体（身份任务及牌局记录）
		    game.countPlayer(current=>{//添加 确定每个玩家的名字
		        var namex=current === game.me ? lib.config.connect_nickname:[
